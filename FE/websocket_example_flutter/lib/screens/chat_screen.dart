...
class ChatScreenState extends State<ChatScreen> {
  ...
  void _listenToMessages() async {
    final history = await client.chat.getChatHistory(widget.user.uid, widget.chatPartner.uid);
    setState(() {
    ...
    _subscription = client.chat.subscribe(widget.user.uid, widget.chatPartner.uid).listen(
          (message) {
  ...
  Future<Uint8List?> _uploadImage(File image) async {
    try {
      ...
      int messageId = await client.chat.uploadImage(
      ...
      // Fetch stored image
      final storedMessage = await client.chat.getMessageById(messageId);
...
// Function to compress the image
  ...
  void _sendMessageStream() async {
    var outStream = client.chat.echoStream(_streamController.stream);
